...
VALUES (@ID, (SELECT ID FROM USERS WHERE EMAIL = 'jj1234@yoohoo.com'), (SELECT ID FROM USERS WHERE EMAIL = 'ben@coldmail.com'), '2019-11-03', 'Interested in a nodejs job?', 0);
SET @ID = (SELECT IF(EXISTS(SELECT 1 FROM CONVERSATIONS), (SELECT MAX(ID) + 1 FROM CONVERSATIONS), 0));
INSERT INTO CONVERSATIONS
VALUES (@ID, (SELECT ID FROM USERS WHERE EMAIL = 'thomJoe@gOgle.com'), (SELECT ID FROM USERS WHERE EMAIL = 'jj1234@yoohoo.com'), '2019-10-17', 'Lets go get ice cream', 0);
SET @ID = (SELECT IF(EXISTS(SELECT 1 FROM CONVERSATIONS), (SELECT MAX(ID) + 1 FROM CONVERSATIONS), 0));
INSERT INTO CONVERSATIONS
VALUES (@ID, (SELECT ID FROM USERS WHERE EMAIL = 'thomJoe@gOgle.com'), (SELECT ID FROM USERS WHERE EMAIL = 'jj1234@yoohoo.com'), '2019-10-21', 'Why does this site have multiple conversations for the same person?', 0);
