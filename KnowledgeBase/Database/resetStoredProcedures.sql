...
DROP PROCEDURE IF EXISTS GET_USER_PER_USERNAME_PASSWORD;
CREATE PROCEDURE GET_USER_PER_USERNAME_PASSWORD (_USERNAME varchar(255), _PASSWORD varchar(255))
BEGIN
    SELECT
        ID,
        USERNAME,
...
CREATE PROCEDURE MODIFY_USER (_ID int, _USERNAME varchar(255), _FIRST_NAME varchar(255), _LAST_NAME varchar(255), _PROFILE_IMAGE_URL varchar(255), _COUNTRY varchar(255), _ABOUT varchar(255))
BEGIN
    SET @ID = 1;
    SET @USERNAME = IFNULL(_USERNAME, (SELECT USERNAME FROM USERS WHERE ID = @ID));
    SET @FIRST_NAME = IFNULL(_FIRST_NAME, (SELECT USERNAME FROM USERS WHERE ID = @ID));
    SET @LAST_NAME = IFNULL(_LAST_NAME, (SELECT LAST_NAME FROM USERS WHERE ID = @ID));
    SET @PROFILE_IMAGE_URL = IFNULL(_PROFILE_IMAGE_URL, (SELECT PROFILE_IMAGE_URL FROM USERS WHERE ID = @ID));
    SET @COUNTRY = IFNULL(_COUNTRY, (SELECT COUNTRY FROM USERS WHERE ID = @ID));
    SET @ABOUT = IFNULL(_ABOUT, (SELECT ABOUT FROM USERS WHERE ID = @ID));
    UPDATE USERS
    SET
        USERNAME = @USERNAME,
...
CREATE PROCEDURE CREATE_USER (_USERNAME varchar(255), _PASSWORD varchar(255), _FIRST_NAME varchar(255), _LAST_NAME varchar(255), _EMAIL varchar(255), _PROFILE_IMAGE_URL varchar(255), _COUNTRY varchar(255), _DATE_OF_BIRTH DATE, _ABOUT varchar(255))
BEGIN
    SET @ID = (SELECT IF(EXISTS(SELECT 1 FROM USERS), (SELECT MAX(ID) + 1 FROM USERS), 1));
    INSERT INTO USERS
    VALUES (@ID, _USERNAME, _PASSWORD, _FIRST_NAME, _LAST_NAME, _EMAIL, _PROFILE_IMAGE_URL, _COUNTRY, _DATE_OF_BIRTH, 0, _ABOUT);
END;
